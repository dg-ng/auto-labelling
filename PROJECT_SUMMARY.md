...
## 10. Cross-Method Cluster Alignment (notebook 10)

### 10.1 Goal

With six clustering methods each producing 16 clusters (arbitrary IDs),
"cluster 3" from MiniLM KMeans and "cluster 7" from RoBERTa DEC are just
labels — they may or may not cover the same articles. This analysis asks:
**do any clusters, across methods and embedding families, genuinely agree
on the same groups of news articles?**

### 10.2 Method — Shared UMAP projection

MiniLM centroids are 384-dim and RoBERTa centroids are 768-dim — directly
incommensurable. To compare them, we concatenate each article's L2-normalised
MiniLM and RoBERTa embeddings into a single 1,152-dim vector, then fit one
UMAP (n_components=50, metric=cosine, random_state=42) on all 3,824 articles.
Every article gets one point in this shared 50-dim space, and every cluster's
centroid — regardless of which method produced it — is the mean of its
member articles' shared-space vectors.
...
centroid(method X, cluster k) = mean(shared_i for i in cluster k)
cosine_sim(centroid_A, centroid_B) = alignment score across families
```

This produces 96 centroids (6 methods × 16 clusters), a 96×96 cosine
similarity matrix, and Hungarian-matched 16×16 alignment sub-matrices for
each of the 15 method pairs (6 choose 2).

### 10.3 Results

| Method pair | Mean best-match | Strong alignments (>0.5) |
|---|---|---|
| All 15 pairs | 0.9995–0.9997 | 16/16 |

**Key finding — UMAP normalisation collapse:** all 96 cosine similarities
fall in the range [0.994, 1.000]. After L2-normalisation, every centroid
points in nearly the same direction in the 50-dim UMAP space — a known
geometric artifact of UMAP when projecting high-dimensional data where
all points land on the same side of the origin.

Practical consequences:
- All 15 heatmaps are near-uniform (values 0.994–1.000 all map to the top
  of the colour scale)
- `strong_alignments = 16/16` for every pair — the 0.5 threshold is always
  cleared, providing no discriminating signal
- The sanity check (same-family > cross-family) still passes: same-family
  pairs (e.g. minilm_kmeans vs minilm_agglomerative) score 0.9997 on average
  vs cross-family pairs at 0.9995 — the ranking is correct but the delta is
  only 0.0002

### 10.4 Interpretation and next steps

The shared UMAP cosine similarity confirms that all six methods are operating
on semantically coherent regions of article space (no method is random), but
cannot distinguish *which* methods agree *more* due to the normalisation
collapse. To get discriminating alignment scores, cosine similarity should be
replaced with **Euclidean distance** in the shared UMAP space, which does not
suffer from the same collapse. Alternatively, mean-centering the 96 centroids
before L2-normalisation would spread the angular distribution.

The 2D centroid scatter (`results/centroid_scatter_2d.png`) and alignment
heatmaps (`results/alignment_heatmap_all.png`) are generated by notebook 10;
`results/centroid_alignment_summary.csv` holds the 15-row summary table.
...
notebooks/
  ...
  10_cross_method_alignment.ipynb   # shared UMAP projection → 96×96 centroid similarity, 15 pairwise heatmaps
...
results/
  ...
  cluster_labels_<method>.npy            # raw integer cluster assignments (used by notebooks 09, 10)
  ...
  centroid_alignment_summary.csv         # 15-row table: method pair × mean best-match × strong alignments
  alignment_heatmap_all.png              # 5×3 grid of 16×16 pairwise heatmaps (notebook 10)
  centroid_scatter_2d.png               # 2D UMAP of all 96 centroids, coloured by method (notebook 10)
```
