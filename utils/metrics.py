...
def hungarian_match_predictions(true_labels, cluster_labels) -> np.ndarray:
    """Return cluster labels mapped to best-matching classes via Hungarian algorithm."""
    true_labels = np.array(true_labels)
    cluster_labels = np.array(cluster_labels)
    ...
    # Build mapping from cluster_id to matched class_id
    cluster_to_class = {}
    for row, col in zip(row_ind, col_ind):
        if row < len(cluster_ids):
            cluster_to_class[cluster_ids[row]] = class_ids[col]

    # Return matched predictions
    matched_predictions = np.array([cluster_to_class.get(c, -1) for c in cluster_labels])
